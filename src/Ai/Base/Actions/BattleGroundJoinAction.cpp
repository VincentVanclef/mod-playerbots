...
bool BGJoinAction::shouldJoinBg(BattlegroundQueueTypeId queueTypeId, BattlegroundBracketId bracketId)
{
    ...
    // RTG: when real players are queued, only let bots at the queue's average level fill it.
    ...
    {
        uint8 targetLevel = sRandomPlayerbotMgr.GetQueueDemandBgTargetLevel(demandLevels, bgInfo.minLevel, bgInfo.maxLevel);
