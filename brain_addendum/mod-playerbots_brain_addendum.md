...
## Chapter: RTG PvP helper readiness gate and preferred level-19 queue normalization
- **Date:** 2026-04-11
- **Subsystem:** mod-playerbots / RTG BG-arena lane readiness
- **Context:** Live mixed BG+arena testing showed helpers still entering the PvP lane as low-level or half-prepared randombots. Some level 1 and level 5 helpers were logged in and counted toward demand while the system waited for enough already-suitable 19s to appear, which delayed actual match launch and polluted the PvP helper pool with non-ready bodies.
- **Finding:** The queue helper pipeline was still treating `logged in` as close enough to `ready` for BG/arena helpers. Arena/BG login only forced level/equipment in a narrow way and could preserve weak prior state. Preferred level-19 queue doctrine was documented in config but not actually wired into adaptive queue level selection.
- **Change:** Added a true PvP helper readiness step for BG/arena login. Preferred queue level activity now raises a runtime `rtg_preferred_pvp_queueing` flag, BG/arena add-data is normalized to the preferred RTG queue level before login, and BG/arena login now rebuilds the helper through `PlayerbotFactory::Randomize(true)` so talents, skills, and gear are brought forward together instead of only nudging level/equipment. Readiness is recorded via `rtg_pvp_profile_ready`, and helpers that cannot become queue-ready are retired instead of proceeding as false positives.
- **Expected proof:** During level-19 BG/arena activity, acquire logs should show preferred queue level add-data instead of low-level leakage; level 1/5 helpers should not survive as queue-ready bodies; queue formation should no longer stall waiting for naturally level-19 bots because the selected helpers are normalized into proper level-19 PvP helpers before dispatch.
